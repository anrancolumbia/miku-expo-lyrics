(() => {
  ...
  function updateOffsetDisplay() {
    ...
    offsetDisplay.textContent = '点击任意一行可跳到该位置';
  }
  ...
  function toggleFullscreen() {
    ...
    if (isFs) {
      ...
      el.requestFullscreen().catch(() => {
        alert('iPhone Safari 不支持全屏 API。\n请点 Safari 的「分享 → 添加到主屏幕」，从图标打开就没有 Safari 栏了。');
      });
    } else if (el.webkitRequestFullscreen) {
      el.webkitRequestFullscreen();
    } else {
      alert('此浏览器不支持全屏。iPhone 请用「分享 → 添加到主屏幕」。');
    }
  }
  ...
  loadSetlist().catch(err => {
    setlistEl.innerHTML = `<li style="color:var(--danger);padding:20px">加载失败: ${err.message}</li>`;
  });
})();
